...
if __name__ == "__main__":
    ...
    for model, layers in MODELS.items():
        ...
        for in_features, out_features in layers:
            ...
            input = torch.randn((1, in_features), dtype=DTYPE)  #  [..., in_features]
            codes = pack_int_data(
                torch.randint(
            ...
            codebooks = torch.randn(
                (args.num_codebooks, 2**args.nbits_per_codebook, 1, args.in_group_size), dtype=DTYPE
            )  #  [num_codebooks, codebook_size, out_group_size, in_group_size]
            scales = torch.randn((out_features, 1, 1, 1), dtype=DTYPE)  #  [num_out_groups, 1, 1, 1]
        ...
        print(f"{model}: Dense GEMV forward = {dense * 1e3:.2f} ms")
        print(f"{model}: AQLM GEMV forward = {quant * 1e3:.2f} ms")
