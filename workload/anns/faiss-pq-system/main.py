...
Examples:
    ...
    parser.add_argument("--algorithm", type=str, default="pq", choices=["pq", "ivf-pq"],
...
def main():
    ...
    try:
        ...
        if args.mode == "build":
            ...
            validate_parameters(args.algorithm, k_values, m_values, 
...
def list_available_datasets():
    ...
    for dataset_name in datasets:
        try:
            ...
            if not exists:
                first_file = list(dataset_info.get('files', {}).values())[0]
...
def run_build_mode(args: argparse.Namespace, k_values: List[int], m_values: List[int]):
    ...
    dist_metric = dataset_info_config.get('params', {}).get('distance_metric', 'l2')
    logger.info(f"Using dataset default distance metric: {dist_metric}")
